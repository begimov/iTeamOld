...
@section('main')
	...
	<div class="block-header">
		...
		<h2><i class="zmdi zmdi-videocam"></i> {{ link_to_route('admin.learn.index','Мастер-проекты/Мастер-классы') }} &rarr; {{ $data->title }}</h2>

		<ul class="actions">
		
			<li>
				<a target="_blank" href="{{ url('learn'.$data->path.$data->wid.'/')  }}">
			...
			@if($data->children)
			<li>
				<a href="{{ route('admin.learn.index') . '?parent_id=' . $data->id }}">
			...
			<li class="dropdown">
				...
				<ul class="dropdown-menu dropdown-menu-right">
					...
					<li>{{ link_to_route('admin.learn.show', $crumb->title, $crumb->id) }}</li>
					@endforeach												
				</ul>
			</li>
			@endif
	...
	<div class="card">
		<div class="card-body card-padding">
			...
			<div role="tabpanel">
				...
				<!--@ yield('form')-->
				{!! Form::model($data, ['route' => ['admin.learn.update',$data->id], 'class' => '', 'files' => true]) !!}
				...
				<div role="tabpanel" class="tab-pane" id="mark">
					...
					<form action="{{ route('admin.resource.add-mark') }}" method="post">
						{{ csrf_field() }}
						<input class="hidden" type="text" name="type_resource" value="3">
...
@section('scripts')
	...
	var configText = {
		...
		contentsCss: ['/css/bootstrap.min.css','/css/company.css'],
		height: 400,
