    ...
    <div class="card">
        <div class="listview lv-bordered lv-lg">
            <div class="lv-header-alt clearfix">
                <h2 class="lvh-label hidden-xs">Метки</h2>
            </div>
            <div>
                <ol>
                    @foreach($marks as $mark)
                        <li>{{ $mark->name }} <a href="{{ route('admin.mark.destroy', ['id' => $mark->id]) }}" class="btn-link before-glyphicon before-glyphicon-danger glyphicon-trash delete"></a></li>
                    @endforeach
                </ol>
            </div>
            <form action="{{ route('admin.mark.store') }}" method="POST">
                {{ csrf_field() }}
                <div class="col-md-6">
                    <input type="text" name="name" placeholder="Название метки" style="width: 100%">
                </div>
