...
@section('main')
	...
	<div class="block-header">
		...
		<h2><i class="zmdi zmdi-edit"></i> {{ link_to_route('admin.article.index','Публикации') }} &rarr; {{ $data->title }}</h2>

		<ul class="actions">
		
			<li>
				<a target="_blank" href="{{ url($data->path.$data->wid.'/')  }}">
			...
			@if($data->children)
			<li>
				<a href="{{ route('admin.article.index') . '?parent_id=' . $data->id }}">
			...
			<li class="dropdown">
				...
				<ul class="dropdown-menu dropdown-menu-right">
					...
					<li>{{ link_to_route('admin.article.show', $crumb->title, $crumb->id) }}</li>
					@endforeach												
				</ul>
			</li>
			@endif
	...
	<div class="card">
		<div class="card-body card-padding">
			...
			<div role="tabpanel">
				...
				<!--@ yield('form')-->
				{!! Form::model($data, ['route' => ['admin.article.update',$data->id], 'class' => '']) !!}
							...
							<div class="col-sm-4 m-t-15">
								<div class="checkbox m-t-15">
									<label {{ $data->children || $data->childrens->count() ? 'class="active"' : '' }}>
										<input type="checkbox" name="children" value="1" {{ $data->children || $data->childrens->count() ? 'checked' : '' }}>
							...
							<div class="col-sm-8">
								...
								<div class="row">
									<div class="col-sm-12">
										...
										{!! Form::control('textarea', 0, 'outro', $errors, 'Блок ссылок', null, null, 'Блок ссылок') !!}
				...
				<div role="tabpanel" class="tab-pane" id="mark">
					...
					<form action="{{ route('admin.resource.add-mark') }}" method="post">
						{{ csrf_field() }}
						<input class="hidden" type="text" name="type_resource" value="1">
