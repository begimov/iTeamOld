<?php
#header("Location: /?404");
header("Content-Type:text/html; charset=utf-8");
?>
        <!DOCTYPE html>
<html>
<head>
    <title>Спасибо за интерес к тесту "{{ $test->name }}" | Консалтинговая компания ITeam</title>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    <meta name="description" content="ITeam" />
    <meta name="keywords" content="ITeam" />

    <link rel="shortcut icon" href="/favicon.ico" />
    <style type="text/css">
        *{background-attachment:scroll;background-color:rgba(0, 0, 0, 0);background-position:95% center;background-repeat:no-repeat;margin:0;padding:0;}
        body{background-color:#EFEFEF;color:#003366;font:13px/16px Tahoma,Arial,sans-serif;margin:0;max-width:1280px;min-height:100%;min-width:960px;padding:0;}
        #body{width:800px;overflow:hidden;}
        #header{position:relative;height:96px;margin:0;width:100%;}
        #logo{height:70px;margin:0 auto;width:140px;}
        .logo{width:200px;height:70px;background:url('data:image/png;base64,') no-repeat scroll 0 -70px transparent;font:70px/70px Arial,Verdana sans-serif;text-indent:201px;display:block;overflow:hidden;border:0 none;text-decoration:none;color:#202;letter-spacing:35px;}
        .logo:hover{background:url('data:image/png;base64,') no-repeat scroll 0 0 transparent;}
        #logo:hover{cursor:default;}
        a.logo:hover{cursor:pointer;}
        #wrapper{clear:both;float:none;}
        s{text-decoration:none;}
        #page{background-color:#FFFFFF;margin:40px auto;padding:40px;width:800px;}
        p{padding:0 0 16px; font-size: 18px; line-height: 1.2}
        p big{color:rgb(204,0,0);font-size:24px;line-height:26px;margin:0 0 32px;display:block;}
        .block{display:block;}

        #main .button_block{display:block;padding:20px 0;position:relative;}
        #main .button_block a{background-color:rgb(221,0,17);border-radius:8px;color:rgb(255,255,255);display:block;font:18px/1 sans-serif;margin:0 auto;padding:8px;text-align:center;text-decoration:none;width:250px;}
        #main .button_block a:hover{background-color:rgb(221,0,0);}
        .more{background-color:rgb(239,239,239);margin:0 0 24px;padding:24px;text-align:center;}
        .more h1,.more h2{color:rgb(204,0,0);line-height:1.4;}
    </style>
    <script type="text/javascript" src="//code.jquery.com/jquery-latest.js"></script>
</head>
<body>
<div id="page">
    <div id="body">
        <div id="header">

            <div id="logo">
                <a class="logo" href="/">ITeam</a>
            </div>

        </div>
        <div id="wrapper">
            <div id="main">
                <p>
                    <big style="text-align:center;">
                        СПАСИБО что прошли тест <br>"<b>{{ $test->name }}</b>"!
                        <br>
                        <br>
                    </big>
                    <a href='javascript:window.print(); void 0;' title="Печатать"><img  src="/filemanager/userfiles/user0/content/pechat.png" /></a>
                </p>

                <p style="font-size: 18px">
                    {!! $result  !!}
                </p>

                <div class="more">
                    <h3><a href="https://iteam.ru/tests/show/{{ $test->id }}">Вернуться на страницу теста</a></h3>
                </div>
            </div>
        </div>
    </div>
</div>

<!-- Yandex.Metrika counter -->
<script type="text/javascript">
    (function (d, w, c) {
        (w[c] = w[c] || []).push(function() {
            try {
                w.yaCounter31848061 = new Ya.Metrika({id:31848061,
                    clickmap:true,
                    trackLinks:true,
                    accurateTrackBounce:true});
            } catch(e) { }
        });

        var n = d.getElementsByTagName("script")[0],
            s = d.createElement("script"),
            f = function () { n.parentNode.insertBefore(s, n); };
        s.type = "text/javascript";
        s.async = true;
        s.src = (d.location.protocol == "https:" ? "https:" : "http:") + "//mc.yandex.ru/metrika/watch.js";

        if (w.opera == "[object Opera]") {
            d.addEventListener("DOMContentLoaded", f, false);
        } else { f(); }
    })(document, window, "yandex_metrika_callbacks");
</script>
<noscript><div><img src="//mc.yandex.ru/watch/31848061" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
<!-- /Yandex.Metrika counter -->


</body>
</html>