	...
	<div class="row">
		<div class="box">
			<div class="col-lg-12">
				...
				<br>
				<small style="display:none;">{{ $post->user->username }} {{ trans('front/blog.on') }} {!! $post->created_at . ($post->created_at != $post->updated_at ? trans('front/blog.updated') . $post->updated_at : '') !!}</small>
				...
				<div class="col-lg-12">
					...
					@if($comments->count())
						@foreach($comments as $comment)
							<div class="commentitem">
								...
								<div id="contenu{!! $comment->id !!}">{!! $comment->text !!}</div>
								<hr>
							</div>
						@endforeach
					@endif	
					...
					<div class="row" id="formcreate"> 
						...
						@else
							...
							<div class="text-center">{!! link_to('auth/login', trans('front/site.connection')) !!}</div>
	...
	<script>	  
		...
		@if(session('statut') != 'visitor')
			...
			CKEDITOR.replace('comments', {
				...
				removeButtons: 'Table,SpecialChar,HorizontalRule,Anchor,CodeSnippet,Image'
			});
